const metrics = [
  ...
  { value: "Automatic", label: "Failure Recovery" },
  { value: "End-to-End", label: "Audit Traceability" },
  { value: "<2s", label: "API Response Target" },
];
...
const cards = [
  {
    ...
    text: "Processes security scans in parallel across growing enterprise environments without interrupting ongoing operations.",
    tone: "cyan",
  },
  {
    number: "02",
    title: "Resilient Operations",
    text: "Automatically manages priorities, retries, schedules, timeouts, and recovery to keep critical security workflows running.",
    tone: "violet",
  },
  {
    number: "03",
    title: "Live Operational Visibility",
    text: "Provides real-time scan progress, status updates, controlled cancellation, and immediate visibility through the dashboard.",
    tone: "emerald",
  },
];
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-cyan-50/75">
            The operational backbone of KAVACH-Q—enabling scalable security
            assessments, real-time progress visibility, automatic recovery,
            and compliance-ready audit traceability.
          </p>
        </div>
        ...
        <div className="mt-16 rounded-4xl border border-cyan-300/20 bg-white/6 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl">
          <div className="grid gap-4 md:grid-cols-3">
            {cards.map((card) => (
              <div
                key={card.title}
                className={`group relative overflow-hidden rounded-3xl border p-7 transition-all duration-300 hover:-translate-y-1 ${getTone(card.tone)}`}
              >
...
function getTone(tone) {
  const tones = {
    cyan: "border-cyan-300/20 bg-cyan-400/10 hover:border-cyan-300/40 hover:bg-cyan-400/15",
    violet: "border-violet-300/20 bg-violet-400/10 hover:border-violet-300/40 hover:bg-violet-400/15",
    emerald: "border-emerald-300/20 bg-emerald-400/10 hover:border-emerald-300/40 hover:bg-emerald-400/15",
  };
